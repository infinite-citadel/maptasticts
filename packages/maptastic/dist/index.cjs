...
class Maptastic {
  ...
  draw() {
    ...
    this.layers.forEach((layer) => {
      if (layer.visible) {
        ...
        context.moveTo(x, y);
        layer.targetPoints.forEach(([x2, y2]) => {
        ...
        layer.targetPoints.forEach((point) => {
          ...
          centerPoint[1] += y2;
          context.beginPath();
          context.arc(x2, y2, this.selectionRadius / 2, 0, 2 * Math.PI, false);
          context.stroke();
        });
        centerPoint[0] /= 4;
        centerPoint[1] /= 4;
  ...
  mouseMove = (event) => {
    ...
    if (this.dragging) {
      ...
      for (var i = 0; i < this.layers.length; i++) {
        ...
        if (layer.visible) {
          for (var p = 0; p < layer.targetPoints.length; p++) {
            ...
            if (distanceTo(point[0], point[1], mouseX2, mouseY2) < this.selectionRadius) {
      ...
      if (this.showCrosshairs || previousState != (this.hoveringPoint != null) || previousLayer != (this.hoveringLayer != null)) {
        this.draw();
      }
    }
  };
  ...
  keyDown = (event) => {
    ...
    var dirty = false;
    const delta = [0, 0];
    tsPattern.match(event).with({ key: " ", shiftKey: true }, () => {
      this.setConfigEnabled(false);
    }).with({ key: tsKeyEnum.Key.ArrowLeft }, () => {
      delta[0] -= increment;
    }).with({ key: tsKeyEnum.Key.ArrowUp }, () => {
      delta[1] -= increment;
    }).with({ key: tsKeyEnum.Key.ArrowRight }, () => {
      delta[0] += increment;
    }).with({ key: tsKeyEnum.Key.ArrowDown }, () => {
      delta[1] += increment;
    }).with({ key: "c" }, () => {
  ...
  addLayer(target, targetPoints) {
    ...
    this.layers.forEach((layer2, n) => {
      if (layer2.element.id == element.id && targetPoints) {
        layer2.targetPoints = clonePoints(targetPoints);
        this.layers[n] = layer2;
      }
    });
    console.log(element);
